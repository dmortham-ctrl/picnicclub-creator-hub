...
export async function getPublishedProfiles(): Promise<Profile[]> {
  ...
  return ((data as Profile[] | null) ?? []).map((profile) => ({ ...profile, level: profile.level ?? (profile.is_featured ? "All Star" : "Rising") }));
}
...
export async function getProfile(username: string): Promise<{ profile: Profile; links: ProfileLink[] } | null> {
  ...
  const { data: profile } = await supabase.from("profiles").select("*").eq("username", username.toLowerCase()).eq("status", "published").maybeSingle();
