...
export default function AdminPage() {
  ...
  // Email verification is a 6-digit code (immune to link pre-fetching by Gmail
  // and security scanners, which was consuming the single-use magic links).
  const [step, setStep] = useState<"form" | "code">("form");
  const [code, setCode] = useState("");
  const [codeType, setCodeType] = useState<"signup" | "email">("signup");
  const [profile, setProfile] = useState({ username: "", display_name: "", bio: "", category: "Lifestyle", avatar_url: "", status: "draft" });
  ...
  function reset() { setMessage(""); setNotice(""); }

  async function sendLoginCode() {
    if (!supabase) return setMessage("Supabase belum dikonfigurasi.");
    reset();
    if (!email) return setMessage("Isi email dulu.");
    const { error } = await supabase.auth.signInWithOtp({ email, options: { shouldCreateUser: false } });
    if (error) return setMessage(/rate limit/i.test(error.message) ? "Terlalu banyak email. Tunggu beberapa menit." : error.message);
    setCodeType("email");
    setStep("code");
    setNotice(`Kode login sudah dikirim ke ${email}. Cek inbox (dan folder spam).`);
  }
  ...
  async function passwordAuth(e: FormEvent) {
    ...
    if (authMode === "login") {
      ...
      if (!error) { setLoggedIn(true); return; }
      if (/email not confirmed/i.test(error.message)) {
        await supabase.auth.resend({ type: "signup", email });
        setCodeType("signup");
        setStep("code");
        setNotice(`Email kamu belum diverifikasi. Kami kirim ulang kode 6 digit ke ${email}.`);
        return;
      }
      return setMessage(/invalid login credentials/i.test(error.message) ? "Email atau password salah." : error.message);
    }
    ...
    if (error) {
      return setMessage(/rate limit/i.test(error.message) ? "Terlalu banyak email dikirim. Tunggu beberapa menit, lalu coba lagi." : error.message);
    }
    ...
    if (data.session) { setLoggedIn(true); return; }
    setCodeType("signup");
    setStep("code");
    setNotice(`Kami kirim kode 6 digit ke ${email}. Masukkan kode itu di bawah untuk mengaktifkan akun.`);
  }
  ...
  async function verifyCode(e: FormEvent) {
    ...
    // onAuthStateChange -> afterAuth handles the redirect to the dashboard.
    setLoggedIn(true);
  }
  ...
  async function resendCode() {
    ...
    if (codeType === "signup") {
      ...
      if (error) return setMessage(/rate limit/i.test(error.message) ? "Terlalu banyak email. Tunggu sebentar." : error.message);
    } else {
      const { error } = await supabase.auth.signInWithOtp({ email, options: { shouldCreateUser: false } });
      if (error) return setMessage(/rate limit/i.test(error.message) ? "Terlalu banyak email. Tunggu sebentar." : error.message);
    }
    setNotice(`Kode baru sudah dikirim ke ${email}.`);
  }
  ...
  async function logout() { await supabase?.auth.signOut(); setLoggedIn(false); reset(); setStep("form"); }
  ...
  if (!loggedIn) {
    return (
      <main className="admin-wrap">
        <div className="admin-topbar"><BrandLogo href={null} /></div>
        <div className="admin-card">
          {step === "code" ? (
            <>
              ...
              <button className="magic-link-button" type="button" onClick={resendCode}>Kirim ulang kode</button>
              <button className="magic-link-button" type="button" onClick={() => { setStep("form"); reset(); setCode(""); }}>← Kembali</button>
              {message && <p className="error">{message}</p>}
            </>
          ) : (
            <>
              <div className="eyebrow">Private workspace</div>
              <h2>{authMode === "login" ? "Welcome back." : "Join Picnic Club."}</h2>
              <p className="hero-copy">{authMode === "login" ? "Login ke dashboard creator Anda." : "Buat akun creator baru di Picnic Club."}</p>
              <div className="auth-tabs">
                <button type="button" className={authMode === "login" ? "active" : ""} onClick={() => { setAuthMode("login"); reset(); }}>Login</button>
                <button type="button" className={authMode === "signup" ? "active" : ""} onClick={() => { setAuthMode("signup"); reset(); }}>Sign up</button>
              </div>
              <button className="button-google" type="button" onClick={loginWithGoogle}>Continue with Google</button>
              <div className="login-divider"><span>atau email</span></div>
              <form className="admin-form" onSubmit={passwordAuth}>
                <label>Email<input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" /></label>
                <label>Password<input type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minimal 8 karakter" /></label>
                {authMode === "signup" && <label>Confirm password<input type="password" required minLength={8} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Ulangi password" /></label>}
                <button className="button-dark" type="submit">{authMode === "login" ? "Login" : "Create account"}</button>
              </form>
              {authMode === "login" && <button className="magic-link-button" type="button" onClick={sendLoginCode}>Kirim kode login ke email</button>}
              {notice && <p className="form-notice">{notice}</p>}
              {message && <p className="error">{message}</p>}
            </>
          )}
  ...
  return (
    <main className="admin-wrap">
      <div className="admin-topbar"><BrandLogo href={null} /><button className="button-outline" type="button" onClick={logout}>Log out</button></div>
      <div className="admin-card">
        <div className="eyebrow">Profiles / new</div>
        <h2>Create creator.</h2>
        <form className="admin-form" onSubmit={save}>
          <div className="admin-row">
            <label>Username<input required value={profile.username} onChange={(e) => setProfile({ ...profile, username: e.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, "") })} placeholder="nama.creator" /></label>
            <label>Display name<input required value={profile.display_name} onChange={(e) => setProfile({ ...profile, display_name: e.target.value })} /></label>
          </div>
          <label>Bio<textarea rows={3} value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} /></label>
          <div className="admin-row">
            <label>Category<input value={profile.category} onChange={(e) => setProfile({ ...profile, category: e.target.value })} /></label>
            <label>Upload avatar<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => setAvatarFile(e.target.files?.[0] ?? null)} /><small style={{ color: "var(--muted)" }}>JPG, PNG, atau WebP. Maks. 5 MB.</small></label>
          </div>
          <button className="button-dark" type="submit" disabled={uploading}>{uploading ? "Menyimpan..." : "Save draft"}</button>
        </form>
        {message && <p className="error">{message}</p>}
      </div>
    </main>
